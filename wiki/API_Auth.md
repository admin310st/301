...
flowchart LR
  ...
  ResetRequest --> Email[[Email с ссылкой]]
  Email --> ClickLink[Пользователь кликает]
  ClickLink --> Verify[/POST \/auth\/verify/]
  Verify --> ResetSession[[reset_session cookie + CSRF token]]
  ResetSession --> PasswordForm[UI: Форма нового пароля]
  PasswordForm --> Confirm[/POST \/auth\/confirm_password/]
  Confirm --> Success[[Пароль изменён]]
  Success --> Login[Redirect на Login]
```
...
```mermaid
flowchart TD
    A[🔐 Забыли пароль?<br/>email + Turnstile] -->|POST /auth/reset_password| B{OAuth-only?}
    
    B -->|Да| C[❌ Показать:<br/>'Войдите через Google/GitHub']
    B -->|Нет| D[📧 Email отправлен<br/>Ссылка 15 мин]
    
    D --> E[👆 Клик по ссылке<br/>/auth/verify?type=reset&token=xxx]
    
    E -->|POST /auth/verify| F[🎫 Получен csrf_token<br/>+ reset_session cookie]
    
    F --> G[🔑 Форма нового пароля<br/>password + confirm]
    
    G -->|POST /auth/confirm_password<br/>password + csrf_token| H{Валидация}
