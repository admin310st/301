...
| Проекты, сайты, домены| Управление зонами и настройками      | `projects`, `sites`, `domains`, `zones`                               |
...
Таблицы redirect_rules и tds_rules связаны с сайтами (site_id) и определяют общие правила для всех доменов сайта
...
erDiagram
    ...
    sites ||--o{ domains : домены
    sites ||--|| zones : зона

    sites ||--o{ redirect_rules : редиректы
    sites ||--o{ tds_rules : распределение
...
| ------------------- | ------------------------------- | --------------------------------- |
| QUEUE_LOGS          | Передача логов редиректов       | `{ site_id, url, ip, ts }`      |
