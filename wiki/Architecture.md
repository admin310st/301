...
| Роль | В схеме БД | Описание | Может быть primary? |
|------|------------|----------|---------------------|
| `acceptor` | `role='acceptor'` | Принимает трафик (лендинг, TDS) | ✅ Да |
| `donor` | `role='donor'` | Редиректит на acceptor | ❌ Нет |
| `reserve` | `role='reserve'` | В резерве, не привязан к сайту | ❌ Нет |
...
| Backend (код/API) | UI (для пользователя) | Описание |
|-------------------|----------------------|----------|
| `zone` | — (скрыто) | Технический контейнер CF |
| `domain` (2-го уровня) | Домен | Root-домен: example.com |
| `domain` (3-го+ уровня) | Поддомен | api.example.com |

> **Правило:** В UI термин "зона" не используется. Пользователь работает с "доменами" и "поддоменами".
...
| Источник | Поля |
|----------|------|
| **Cloudflare** | `ns`, `ns_verified`, `proxied`, `ssl_status` |
| **301.st** | `role`, `blocked`, `blocked_reason`, `site_id`, `parent_id` |

Пример:
При запросе GET /domains API возвращает объединённые данные + информацию о связанном сайте/проекте.
